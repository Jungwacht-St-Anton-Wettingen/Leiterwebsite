...
def create_ics_event(summary, start_date, end_date=None, location=None, description=None, time=False, color=False):
    event = Event()
...
def create_ics_file(events, org_name):
    ...
    cal.add('version', '2.0')
    cal.add('X-WR-CALNAME', 'Leiter Agenda')
...
if __name__ == "__main__":
    ...
    for category, events in events_data.items():
        for event in events:
            event["origin"] = category  # Add a new attribute "origin" to store the category
            all_events.append(event)

    # Sort events by start_date
    sorted_events = sorted(all_events, key=lambda x: x["start_date"])

    # Assign a color to each origin
    colors = {"Jungwacht": "#FF0000", "Gemeinsamer": "#0000FF", "Externer": "#008000"}

    # Add a new attribute "color" to each event
    for event in sorted_events:
        event["color"] = colors[event["origin"]]
    ics_events = []
    ...
    # Save to a file
    with open(path.join(folder_path, 'leiter_agenda.ics'), 'wb') as f:
        f.write(ics_content)
